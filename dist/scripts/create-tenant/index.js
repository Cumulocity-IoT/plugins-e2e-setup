...
function createApiClient(host, auth) {
    async function req(path, params = {}) {
        ...
        try {
            ...
            const response = await fetch(host + path, {
                ...reqObj,
                body: reqObj.method !== 'GET' && reqObj.body
...
async function createTenant({ tenantName, managementUrl, user, password, email, managementUser, managementPassword, appsToSubscribe, isManagement = true, noTenantSuffix = false, companyName = 'e2eTesting tenant', contactName = 'Mr. Smith', numberOfTenants = 1 }) {
    ...
    const c8yapi = (0, api_1.createApiClient)(managementUrl, {
        user: managementUser,
    ...
    const getApplicationCollection = async () => {
        const url = '/application/applicationsByOwner/management?pageSize=1000&withTotalPages=true';
        const resp = await c8yapi.req(url);
        return resp.applications.map(elem => lodash_1.default.pick(elem, ['owner', 'name', 'id']));
    };
    const getListOfPermissions = async (tenantId) => {
        ...
        const res = await c8yapi.req(url, {
            headers: {
                Authorization: `Basic ${Buffer.from(`${tenantId}/${user}:${password}`).toString('base64')}`
            }
        });
    ...
    const getAdminRoleId = async (tenantId) => {
        ...
        const adminGroup = lodash_1.default.find(resp.groups, { name: 'admins' });
        if (!adminGroup)
        ...
        try {
            ...
            for (const permission of permissionsList) {
                try {
                    ...
                    if (error.status === 409) {
                        console.log(`Permission ${permission} already exists for tenant ${tenantId}, skipping...`);
                        continue;
                    }
                    throw error;
                }
    ...
    const findSubscriptionsIds = async () => {
        ...
        return apps
            .filter(app => applicationsToBeSubscribed.includes(app.name) &&
    ...
    const subscribeMicroservice = async (idsToSubscribe, tenantId) => {
        for (const id of idsToSubscribe) {
            try {
                ...
                if (error.status === 409) {
                    console.log(`Microservice ${id} already subscribed for tenant ${tenantId}, skipping...`);
                    continue;
                }
                console.error(`Failed to subscribe microservice ${id} for tenant ${tenantId}:`, error);
                throw error;
            }
        }
    };
    try {
        ...
        if (!lodash_1.default.includes(tenantIds, undefined) &&
